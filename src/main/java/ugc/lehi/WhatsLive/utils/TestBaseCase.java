...
public class TestBaseCase {
	...
	@BeforeTest
	@Parameters({ "driverName", "nodeURL", "appName", "deviceId", "deviceName", "sdkVersion", "appMainPackage",
			"appActivity", "platformName" })
	public void setup(String driverName, String nodeURL, String appName, String deviceId, String deviceName,
			String sdkVersion, String appMainPackage, String appActivity, String platformName)
		...
		try {
			String cmd = "appium -a " + nodeURL.split(":")[0] + "  -p " + nodeURL.split(":")[1] + " -U " + deviceId;
		...
		if (nodeURL.equals("") || nodeURL.isEmpty()) {
			...
			log.info("读取xml配置：Mobile Driver:" + driverName + "；Appium Server:" + "http://" + nodeURL + "/wd/hub"
					+ "设备Id：" + deviceId + "设备名字：" + deviceName);
			try {
				// Process process=Runtime.getRuntime().exec("cmd.exe /c
				// appium");
				// System.out.println(process.toString());
				// this.driver=setRemoteDriver(driverName, nodeURL, appName,
				// deviceName, sdkVersion, appMainPackage,
				// appActivity,platformName);
				driver = setRemoteDriver(driverName, nodeURL, appName, deviceId, deviceName, sdkVersion, appMainPackage,
						appActivity, platformName);
	 ...
	 *            app主类名
	 * @param platformName
	 *            设备平台 真机：Android AVD虚拟设备：Android Emulator
	 * @return
	 * @throws MalformedURLException
	 */
	private AndroidDriver<?> setRemoteDriver(String driverName, String nodeURL, String appName, String devieceId,
			String deviceName, String sdkVersion, String appMainPackage, String appActivity, String platformName)
			throws MalformedURLException {
		// 获取app路径
